...
import mindspore.dataset.vision as P
import mindspore.dataset.transforms as P2
from mindspore.communication.management import init, get_rank, get_group_size
from PIL import ImageFilter, Image
import random


def create_dataset_moco(dataset_path, aug_plus, repeat_num=1, batch_size=32, target="Ascend", workers=128, distribute=False):
    """
    ...
    #type_cast_op = P2.TypeCast(ms.int32)
    ...
    def delete_label(x, y):
        return x

    data_set = data_set.map(operations=delete_label, input_columns=["image", "label"], output_columns=["image"], 
                            column_order=["image"], num_parallel_workers=get_num_parallel_workers(trans_work_num))
    data_set = data_set.map(operations=P2.Duplicate(), input_columns="image", output_columns=["im_q", "im_k"],
                            column_order=["im_q", "im_k"], num_parallel_workers=get_num_parallel_workers(trans_work_num))
    data_set = data_set.map(operations=trans, input_columns="im_q", num_parallel_workers=get_num_parallel_workers(trans_work_num))
    data_set = data_set.map(operations=trans, input_columns="im_k", num_parallel_workers=get_num_parallel_workers(trans_work_num))
...
def create_dataset_lincls(dataset_path, do_train, repeat_num=1, batch_size=32, target="Ascend", workers=32, distribute=False):
    ...
    if do_train:
        trans = [
            P.RandomCropDecodeResize(image_size),
            P.RandomHorizontalFlip(),
            P.Normalize(mean=mean, std=std),
            P.HWC2CHW()
        ]
